...
- `src/ui/**`：UI（popup/app/inpage），只放组件/样式/DOM 面板；设置页由 `src/ui/settings/SettingsScene.tsx` 统筹，窄屏走 `SettingsTopTabsNav.tsx`、宽屏走 `SettingsSidebarNav.tsx`，Inpage 相关设置集中在 `src/ui/settings/sections/InpageSection.tsx`，视频字幕采集入口集中在 `src/ui/settings/sections/VideosSection.tsx`，包括阅读风格和 anti-hotlink 域名编辑器
...
npm run test         # vitest（如存在）
npm run build        # 构建产物
npm run check        # 产物校验（manifest/icons 等）
```
