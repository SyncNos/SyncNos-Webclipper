...
| comments | `src/services/comments/` + `src/ui/comments/react/` | article 讨论线程；service-owned DTO、父子不变量、递归删除、唯一 thread graph、V1/V2 locator/archive 兼容，以及 shared App/Inpage panel | `domain/comment-dto.ts`, `domain/comment-thread-graph.ts`, `domain/comment-archive.ts`, `data/storage-idb.ts`, `ThreadedCommentsPanel.tsx` |
