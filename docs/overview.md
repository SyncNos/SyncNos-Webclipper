...
5. 当评论侧栏已打开时，正文选区在 `selectionchange` 后由 pointer/key commit 自动附加到根评论引用；panel 内选区与 reply 输入框不会覆盖 root quote。评论只对全局唯一 exact Range 定位，失败时显示 notice，不做近似滚动。
...
| **评论操作需防重入且保持 root-only 选区语义** | Comments controller + React reducer | 评论保存/回复/删除如果并发会引发竞态和重复提交；reply 输入误触会污染引用区 | mutation generation 丢弃旧 context 的晚到 completion；reducer 管理 submit/delete/focus；selection hook 只更新根评论 attachment |
