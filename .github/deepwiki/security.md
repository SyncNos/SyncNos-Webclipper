...
| Feishu OAuth client secret（可选） | 用户配置 | `chrome.storage.local` | 备份时显式排除 `feishu_oauth_client_secret`；若未配置则可通过 Worker 完成 exchange/refresh（扩展端不持有 secret） |
...
| 令牌交换（Feishu） | 扩展直连或 Cloudflare Worker | 若配置 `feishu_oauth_client_secret` 则扩展直连 Feishu token endpoint；否则通过 Worker exchange/refresh（避免在扩展端存 secret） |
...
| 控制点 | 文件 | 说明 |
| --- | --- | --- |
| 敏感键排除 | `src/services/sync/backup/backup-utils.ts` | denylist 排除 `notion_oauth_token*`、`feishu_oauth_token*`、`notion_oauth_client_secret`、`feishu_oauth_client_secret` |
