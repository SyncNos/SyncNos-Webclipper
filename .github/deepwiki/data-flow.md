...
2. orchestrator 读取 `feishu_oauth_token_v1` 并在接近过期时通过 token exchange/refresh worker 刷新；扩展端不存 `client_secret`。
...
5. 写入策略：优先使用 Convert API 把 markdown 转换为 DocX blocks；若权限不足或转换失败，则回退为纯文本 blocks；当 markdown 中包含图片时，会尝试上传并插入图片（失败则回退为纯文本 blocks 写入）。
...
- `src/services/sync/sync-provider-gate.ts`
