...
| 路径 | 角色 | 典型内容 |
| --- | --- | --- |
| `macOS/` | 已归档历史资料 | `SyncNos/`, `SyncNos.xcodeproj/`, `Packages/` |
| `./` | 浏览器扩展 | `src/entrypoints/`, `src/collectors/`, `src/services/`, `src/ui/` |
| `.github/workflows/` | CI / Release / 商店发布 | `release.yml`, `webclipper-*.yml` |
| `.github/scripts/webclipper/` | 打包 / 发布脚本 | release assets、AMO source |

## 关键入口文件

| 入口 | 路径 | 作用 |
| --- | --- | --- |
| 后台入口 | `src/entrypoints/background.ts` | 消息路由、sync orchestrator、OAuth 监听 |
| 内容入口 | `src/entrypoints/content.ts` | collectors registry、inpage UI、runtime observer |
| 设置入口 | `src/ui/settings/SettingsScene.tsx` | General / Chat with AI / Backup / Notion / Feishu / Obsidian / Inpage / Insight / About |
| WXT 入口 | `wxt.config.ts` | 版本号、权限、host permissions |
| 主题 tokens | `src/ui/styles/tokens.css` | `prefers-color-scheme` 驱动亮/暗 token |
...
| 要改扩展的本地统计、Settings Insight 或分布图 | [modules/webclipper.md](modules/webclipper.md), [storage.md](storage.md) | 这些页面一起回答"统计从哪来、限制是什么"。 |
...
| 要查为什么配置没生效 | [configuration.md](configuration.md) | 最接近真实错误发生点。 |
