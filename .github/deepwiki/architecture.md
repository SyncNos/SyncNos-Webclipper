...
| sync | `src/services/sync/` | Notion / Obsidian / Feishu / 备份的编排层（含评论数同步） | `notion-sync-orchestrator.ts`, `obsidian-sync-orchestrator.ts`, `feishu/feishu-sync-orchestrator.ts`, `backup/*`, `comment-metrics.ts` |
