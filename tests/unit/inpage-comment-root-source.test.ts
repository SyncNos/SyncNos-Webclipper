...
describe('inpage comment root source', () => {
  test('derives the smallest stable capture root from the current selection', () => {
    const document = new JSDOM('<main><article><p>Hello <b>world</b></p></article></main>', { url: 'https://example.com/' }).window.document;
  ...
  test('rejects panel selections and body-only capture fallback', () => {
    const document = new JSDOM('<body><div id="panel">panel</div><span>page</span></body>', { url: 'https://example.com/' }).window.document;
    const panel = document.querySelector('#panel')!;
    const range = document.createRange(); range.selectNodeContents(panel);
    const selection = { rangeCount: 1, getRangeAt: () => range } as unknown as Selection;
  ...
  test('restores document-relative root first and limits evidence candidates', () => {
    const document = new JSDOM('<main><article>exact</article><article>other</article></main>', { url: 'https://example.com/' }).window.document;
