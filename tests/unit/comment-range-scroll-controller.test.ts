...
function container(rect: { top: number; bottom: number }, top = 0, height = 1000, client = 100) {
  ...
  const value: CommentScrollContainer = {
    ...
    scrollTo: (next, behavior) => { scrollTop = next; calls.push({ top: next, behavior }); },
  };
  return { value, calls };
}
...
describe('scrollExactCommentRange', () => {
  test('applies the minimum delta through nested scroll containers', () => {
    ...
    const result = scrollExactCommentRange({ range, containers: [inner.value, outer.value], viewportRect: { top: 0, bottom: 200, left: 0, right: 200 } });
  ...
  test('does nothing when exact range is visible', () => {
    ...
    expect(scrollExactCommentRange({ range, containers: [current.value], viewportRect: { top: 0, bottom: 200, left: 0, right: 200 } })).toEqual({ scrolled: 0 });
  ...
  test('uses auto behavior for reduced motion and clamps scroll bounds', () => {
    ...
    scrollExactCommentRange({ range, containers: [current.value], viewportRect: { top: 0, bottom: 100, left: 0, right: 100 }, reducedMotion: true });
